...
function screen(ox, oy) {
  ...
  for (let x = 0; x < w; x += a) {
    for (let y = 0; y < h; y += a) {
      ...
      const eps = 0.00001;
      const derX = (metaValue(ox + x + eps, oy + y) - value) / eps;
      const derY = (metaValue(ox + x, oy + y + eps) - value) / eps;
      const angle = Math.atan2(derY, derX)
      const len = Math.hypot(derX, derY)
      //var value3 = metaValue(ox + x, oy + y, 0.5)
      //console.log(Math.exp(value * 10000));
      var c = HSIToRGB({
        h: angle, s: .5, i: (2 ** -(1 / len * 20)) * 0.7
      })

      /*var c = HSIToRGB({
        h: len / 5, s: 0.5, i: 0.5// angle * 2
      })*/
      c = clampColor(c, "clampSaturation")
      fill(toSRGB(c.r + Math.random() * 0.001) * 255, toSRGB(c.g + Math.random() * 0.001) * 255, toSRGB(c.b + Math.random() * 0.001) * 255);
      square(x + a / 2, y + a / 2, a);
    }
  }
